...
processPullRequest pr mAuth = do
    ...
    _ <- (flip finally) (shelly $ rm_rf $ "/tmp/" ++ (toString rName)) $ shelly $ do
        ...
        liftIO $ putTextLn $ "base branch: " <> baseBranch
        liftIO $ putTextLn $ "head branch: " <> prBranch
...
makeNewbranchAndCommit brName = do
    ...
    _ <- run "git" ["add", "."]
    _ <- run "git" ["commit", "--author=\"nixfmt <r.stryungis@gmail.com>\"", "-m", "Nix formatter bot changes.\n\nFiles were changed by Nix formatter bot."]
